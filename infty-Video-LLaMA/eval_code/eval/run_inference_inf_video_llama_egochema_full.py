    ...
    with open(file_path, 'r') as json_file:
        ...
        for qa_key in tqdm(questions):
            if qa_key["q_uid"] not in result_data: #and qa_key["q_uid"] in answers:
                ...
                video_list = get_video_fragments(video_folder + '/' +qa_key["q_uid"] + ".mp4", num_frames, args.n_samples, args.task)

                question = qa_key['question']
                options = [qa_key["option 0"], qa_key["option 1"], qa_key["option 2"], qa_key["option 3"], qa_key["option 4"]]
                llm_message = process_qa(chat, video_list, question, options, args.num_beams, args.temperature, args.n_samples, args.task)
