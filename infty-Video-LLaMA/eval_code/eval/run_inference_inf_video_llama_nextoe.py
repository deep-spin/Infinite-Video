...
def process_json_file(file_path, chat, args, video_folder, output_file):
    ...
    for index, qa_key in questions.iterrows():
        if not str(qa_key["video"]) + "/"  +str(qa_key["video"]) in list(result_data.keys()):
            ...
            llm_message = process_qa(chat, video_list, question, [], args.num_beams, args.temperature, args.task)
