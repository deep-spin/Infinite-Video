...
from .Qformer_baseline import BertConfig, BertLMHeadModel_baseline
...
class Blip2Base(nn.Module):
    ...
    def init_Qformer(
        cls, 
        num_query_token, vision_width, tau, alpha, sticky, num_basis, baseline, 
        ...
        if baseline:
            Qformer = BertLMHeadModel_baseline(config=encoder_config)
        else:
